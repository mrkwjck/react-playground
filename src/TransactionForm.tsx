...
export default function TransactionForm({
    ...
    const transactionCreation = useMutation({
        mutationFn: (transactionRequest: TransactionRequest) => {
            return axios.post(
                `http://localhost:8080/api/v1/accounts/` + accountNumber + "/" + transactionEndpointPath,
                transactionRequest,
            );
        },
        onSuccess: () => {
    ...
    return (
        <>
            ...
            <h3>{accountNumber}</h3>
            <p>
                <input onChange={(event) => setTransactionAmount(event.target.value)} value={transactionAmount} />
            </p>
            <div className="actions">
